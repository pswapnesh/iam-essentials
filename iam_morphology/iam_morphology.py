...
def iam_binary_dilation(binary: LabelsData, disk_size :int = 1) -> LabelsData:
    return morph.binary_dilation(binary,morph.disk(disk_size))

def iam_binary_erosion(binary: LabelsData, disk_size: int = 1)-> LabelsData:
    return morph.binary_erosion(binary,morph.disk(disk_size))    

def iam_binary_closing(binary: LabelsData, disk_size: int = 1)-> LabelsData:
    return morph.binary_closing(binary,morph.disk(disk_size))    

def iam_binary_opening(binary: LabelsData, disk_size:int = 1)-> LabelsData:
    return morph.binary_opening(binary,morph.disk(disk_size))    

def iam_distance_transform(binary: LabelsData)-> LabelsData:
